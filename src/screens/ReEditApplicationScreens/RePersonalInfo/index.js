...
export default function RePersonalInfo(props) {
	const { contaxtprofileData } = useCart();

	const [Gender, onGender] = React.useState(contaxtprofileData?.gender ? contaxtprofileData.gender : 'Male');
	const [SurName, onSurName] = React.useState(contaxtprofileData?.surName ? contaxtprofileData.surName :'');
	const [FullName, onFullName] = React.useState(contaxtprofileData?.fullName? contaxtprofileData.fullName:'');
	const [IdNumber, onIdNumber] = React.useState(contaxtprofileData?.idNumber? contaxtprofileData.idNumber:'');
	const [MobileNumber, onMobileNumber] = React.useState(contaxtprofileData?.mobileNumber? contaxtprofileData.mobileNumber:'');
	const [Region, onRegion] = React.useState('');
	const [ResidentialTown, onResidentialTown] = React.useState(contaxtprofileData.town?contaxtprofileData.town:'');
	const [StreetName, onStreetName] = React.useState(contaxtprofileData.streetName?contaxtprofileData.streetName:'');
	const [PhysicalAddress, onPhysicalAddress] = React.useState(contaxtprofileData.physicalAddress?contaxtprofileData.physicalAddress:'');
	...
	const SignupCommand = () => {
		...
		props.navigation.navigate('ReEmployer', {
				ReferralCode: contaxtprofileData?.referralCode,
						...
						<View style={{ marginTop: 15 }}>
							...
							<View style={styles._input_view}>
								...
								<TextInput
									...
									value={MobileNumber}
									style={styles._input}
									editable={!contaxtprofileData?.mobileNumber} // Disable if mobileNumber exists
